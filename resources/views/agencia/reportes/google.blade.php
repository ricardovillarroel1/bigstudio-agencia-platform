<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display text-xl font-bold text-gray-800 leading-tight">
            <span class="text-brand-600">Reporte</span> de Performance — Google Ads
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bs-card p-4">
                <div class="flex flex-wrap gap-4 items-end">
                    ...
                    <form method="GET" action="{{ route('agencia.reportes.google') }}" class="flex flex-wrap gap-3 items-end">
                    ...
                    @if($cuenta)
                    <form method="POST" action="{{ route('agencia.reportes.google.conexion.sincronizar', $cuenta) }}" class="flex items-end">
                        @csrf
            ...
            @if(!$cuenta)
                <div class="bs-card p-12 text-center">
                    <div class="inline-flex w-16 h-16 rounded-2xl mb-3 items-center justify-center" style="background: linear-gradient(135deg, #FFF7EC 0%, #FFEDD0 100%);">
                        <i class="fab fa-google text-2xl text-brand-500"></i>
                    </div>
                    <p class="text-gray-600 m-0">No hay cuentas vinculadas todavía.</p>
                    <a href="{{ route('agencia.reportes.conexion') }}" class="bs-link text-sm mt-2 inline-block">Ir a Conectar Google Ads</a>
                </div>
            @elseif(!$resumen)
                <div class="bs-card p-12 text-center">
                    ...
                    <form method="POST" action="{{ route('agencia.reportes.google.conexion.sincronizar', $cuenta) }}">
                        @csrf
                        <input type="hidden" name="periodo" value="{{ $periodo }}">
            ...
            @else
                ...
                <div class="bs-card overflow-hidden">
                    <div class="px-8 py-6 flex items-center justify-between flex-wrap gap-4" style="background: linear-gradient(135deg, #FFC800 0%, #FF9C00 50%, #FF8100 100%);">
                        <div>
                            ...
                            <p class="text-sm text-white/90 mt-1 mb-0"><i class="far fa-calendar"></i> {{ ucfirst($mesNombre) }} · Google Ads</p>
                        </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bs-card p-5">
                        ...
                        <p class="text-xs text-gray-500 mt-1 mb-0">Costo x compra: {{ $fmt($cpa) }} @if(!empty($comparativas['cpa'])) {!! $renderDelta($comparativas['cpa']) !!} @endif</p>
                    </div>
                    <div class="bs-card p-5">
                        <div class="flex items-center justify-between">
                ...
                <div class="bs-card overflow-hidden">
                    ...
                    <div class="bs-card-body">
                        <form method="POST" action="{{ route('agencia.reportes.google.conexion.envio', $cuenta) }}" id="form-auto-envio">
                            @csrf
                ...
                <p class="text-xs text-gray-400 text-center">Datos sincronizados desde Google Ads · última actualización {{ $cuenta->ultima_sync_at ? $cuenta->ultima_sync_at->format('d/m/Y H:i') : '—' }}</p>
                ...
                <dialog id="modal-envio" style="border:none; border-radius:16px; padding:0; max-width:560px; width:92%; box-shadow:0 20px 60px rgba(0,0,0,0.2);">
                    <form method="POST" action="{{ route('agencia.reportes.google.conexion.enviar-ahora', $cuenta) }}" style="padding:28px;" id="form-envio-reporte">
                        @csrf
                        <input type="hidden" name="periodo" value="{{ $periodo }}">
